...
def load_applications_data():
    ...
    data['Proposed EPA Funding'] = data['Proposed EPA Funding'].replace('[\$,]', '', regex=True).astype(float) * 1000  # Clean and adjust funding values
    return data
...
def load_awards_data():
    ...
    data = pd.read_csv(url)
    data['Amount Awarded'] = data['Amount Awarded'].replace('[\$,]', '', regex=True).astype(float) * 1000  # Clean and adjust awarded values
    return data
...
def filter_applications_by_state(data, state):
    ...
    state_abbr = {v: k for k, v in state_names.items()}
    selected_state_abbr = state_abbr[state]
    return data[data['Project State(s)'].str.contains(selected_state_abbr)]
...
def visualize_applications():
    ...
    state_options = list(state_names.values())
